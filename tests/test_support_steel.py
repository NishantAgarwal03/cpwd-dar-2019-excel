"""Tests for Sub-Head 10 Steel Work First-Principles Sheet."""
...
class TestSupportSteel(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.wb = openpyxl.load_workbook(WORKBOOK_PATH, data_only=True)
        cls.ws = cls.wb["10_Steel_Work"]
    ...
    def test_sheet_exists(self):
        self.assertIn("10_Steel_Work", self.wb.sheetnames)

    def test_title_and_columns(self):
        title = self.ws.cell(1, 1).value
        self.assertIn("STEEL WORK", title)
    ...
    def test_core_items_present(self):
        ...
        for r in range(7, self.ws.max_row + 1):
            ...
            if val and str(val).strip().startswith("10."):
                found_items.add(str(val).strip())

        key_items = {
            "10.1", "10.2", "10.3", "10.5", "10.6",
            "10.14.1", "10.16.1", "10.22", "10.25", "10.28"
        }
        for ki in key_items:
            self.assertIn(ki, found_items, f"Missing key item {ki} in 10_Steel_Work")
    ...
    def test_direct_materials_and_references_present(self):
        ...
        self.assertTrue(len(references) > 0, "Expected referenced base items (e.g. REF#13.50.3, REF#4.1.8)")
        self.assertTrue(len(materials) > 0, "Expected direct materials (structural steel, tubes, SS-304, etc.)")
